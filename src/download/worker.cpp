...
void speed::start() {
	...
	cout << "Ожидайте завершения загрузки... (Для отмены нажмите q)" << endl;
	draw(0.0, 0);
...
void speed::draw(double progress, unsigned int speed) {
	...
	if (speed > 0 && total > received) {
		unsigned long long remaining_bytes = total - received;
		unsigned int remaining_seconds = remaining_bytes / speed;
		cout << " | Осталось: ";
		if (remaining_seconds < 60) {
			cout << remaining_seconds << " сек";
		} else if (remaining_seconds < 3600) {
			cout << remaining_seconds / 60 << " мин " << remaining_seconds % 60 << " сек";
		} else {
			cout << remaining_seconds / 3600 << " ч " << (remaining_seconds % 3600) / 60 << " мин";
		}
	}
